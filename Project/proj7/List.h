...
bool List<Item>::insertAfter(Item otherPersonsName, Item yourName) {
	for (Node *ptr = myFirst; ptr; ptr = ptr->myNext) {
		if (ptr->myItem == otherPersonsName) {
			ptr->myNext = new Node(yourName, ptr->myNext);
			if  (ptr->myItem == myLast->myItem) {
				myLast = ptr->myNext;
			}
			mySize ++;
			return true;
...
bool List<Item>::insertBefore(Item otherPersonsName, Item yourName) {
	if (myFirst->myItem == otherPersonsName) {
		myFirst = new Node(yourName, myFirst);
		mySize ++;
		return true;
	} else {
		for (Node *ptr = myFirst; ptr; ptr = ptr->myNext) {
			if (ptr->myNext->myItem == otherPersonsName) {
				ptr->myNext = new Node(yourName, ptr->myNext);
				mySize ++;
				return true;
			}
...
template<class Item>
ostream& operator<<(ostream &out, List<Item> myLine) {
	myLine.writeTo(out, '=');
	return out;
}
