...
void ListTester::testInsertAfter() {
	...
	assert(list0.myLast->myItem == 0);
	cout << " 1 " << flush;

	assert(list0.insertAfter(0, 1) == true);      // Return true when successfully inserted.
	assert(list0.mySize == 2);
	assert(list0.myFirst->myItem == 0);
	assert(list0.myLast->myItem == 1);
	cout << " 2 " << flush;

	assert(list0.insertAfter(1, 2) == true);      // Insert after the last item.
	assert(list0.mySize == 3);
	assert(list0.myFirst->myItem == 0);
	assert(list0.myLast->myItem == 2);
	cout << " 3 " << flush;

	assert(list0.insertAfter(1, 1.5) == true);    // Insert in the middle of the list.
	assert(list0.mySize == 4);
	assert(list0.myFirst->myItem == 0);
	assert(list0.myLast->myItem == 2);
	cout << " 4 " << flush;

	assert(list0.insertAfter(4, 5) == false);     // Return false when the item doesn't exist.
...
void ListTester::testInsertBefore() {
	...
	assert(list1.myLast->myItem == 0);
	cout << " 1 " << flush;

	assert(list1.insertBefore(0, 1) == true);      // Return true when successfully inserted.
	assert(list1.mySize == 2);
	assert(list1.myFirst->myItem == 1);
	assert(list1.myLast->myItem == 0);
	cout << " 2 " << flush;

	assert(list1.insertBefore(1, 2) == true);      // Insert before the first item.
	assert(list1.mySize == 3);
	assert(list1.myFirst->myItem == 2);
	assert(list1.myLast->myItem == 0);
	cout << " 3 " << flush;

	assert(list1.insertBefore(1, 1.5) == true);    // Insert in the middle of the list.
	assert(list1.mySize == 4);
	assert(list1.myFirst->myItem == 2);
	assert(list1.myLast->myItem == 0);
	cout << " 4 " << flush;

	assert(list1.insertBefore(3, 4) == false);     // Return false when the item doesn't exist.
...
void ListTester::testOutput() {
	...
	list2.insert("Ann", 0);
	list2.insert("Bob", 1);
	list2.insert("Carol", 2);
	cout << " 0 " << flush;

	ofstream testFile1;
	testFile1.open ("testOutPut.txt");
	testFile1 << list2;
	testFile1.close();
	cout << " 1 " << flush;
	...
	ifstream testFile2;
	ostringstream s1;
	testFile2.open ("testOutPut.txt");
	for (string line; getline(testFile2, line); ) {
		s1 << line;
	}
	string s2 = s1.str();
	cout << " 2 " << flush;
	...
	cout << "Passed: the list2 and s2 print out the same string" << endl;
}
