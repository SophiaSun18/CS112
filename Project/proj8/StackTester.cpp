...
void StackTester::testGetSizeGetCapacityAndSetCapacity() {
	cout << "- Testing getSize() and getCapacity()... " << flush;
	Stack<int> s1(5);
	s1.push(11);
	s1.push(22);
	s1.push(33);
	...
	// test with new capacity = 0
	try {
		s1.setCapacity(0);
		cerr << "\nsetCapacity() worked with an invalid new capacity\n";
		exit(1);
	} catch (const StackException& se) {
		cout << " 1 " << flush;
	}
	// test with new capacity < mySize
	try {
		s1.setCapacity(1);
		cerr << "\nsetCapacity() worked with an invalid new capacity\n";
		exit(1);
	...
	// test with a proper new capacity
	s1.setCapacity(7);
	assert(s1.getSize() == 3);
