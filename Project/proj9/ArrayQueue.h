...
private:
	...
	friend class PalindromeDetector;
...
void ArrayQueue<Item>::setCapacity(unsigned newCapacity) {
	if (newCapacity == 0 or newCapacity < getSize()) {
		throw QueueException("setCapacity()", "bad new capacity!");
	} else {
		Item* newArray = new Item[newCapacity];
		if (myFirstIndex <= myLastIndex) {
			// no wrap-around
			for (unsigned i = 0; i < mySize + 1; i++) {
				newArray[i] = myArrayPtr[myFirstIndex + i];
			}
		} else if (myFirstIndex > myLastIndex) {
			// with wrap-around
			// divide the original queue to 2 parts and copy separately
			for (unsigned i = 0; i < myCapacity - myFirstIndex + 1; i++) {
				// from myFirstIndex to the end
				newArray[i] = myArrayPtr[myFirstIndex + i];
			}
			for (unsigned i = 0; i < myLastIndex + 1; i++) {
				// from the beginning to myLastIndex
				newArray[myCapacity - myFirstIndex + i] = myArrayPtr[i];
			}
		}
		delete [] myArrayPtr;
		myArrayPtr = newArray;
...
template <class Item>
void ArrayQueue<Item>::print() {
	for (int i = 0; i < mySize; i++) {
		cout << "i: " << myArrayPtr[i] << endl;
	}
}
