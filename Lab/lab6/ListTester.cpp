...
void ListTester::testGetIndexOf() {
	...
	assert( aList.myFirst->myItem == 10 );
	assert( aList.myLast->myItem == 50 );
	cout << " 1 " << flush;

	assert(aList.getIndexOf(10) == 0);
	assert(aList.getIndexOf(40) == 3);
	assert(aList.getIndexOf(100) == -1);
	cout << " 2 " << flush;
	cout << "Passed!" << endl;
}
...
void ListTester::testEquality() {
	...
	assert((l1 == l2) == true);
	assert((l1 == l3) == false);   // The test fails because the values are different.
	assert((l1 == l4) == false);   // The test fails because the number of values are different.
	cout << "Passed!" << endl;
}
